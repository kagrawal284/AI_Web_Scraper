...
# ) -> str:
#     """
#     Parse DOM chunks using Google Gemini 1.5 Flash
...
#         print(f"🚀 Processing {total_chunks} chunks with Gemini 1.5 Flash...")
        
#         for i, chunk in enumerate(dom_chunks, start=1):
#             try:
#                 print(f"⚡ Processing chunk {i}/{total_chunks} (length: {len(chunk)})")
...
#                 # Create prompt
#                 prompt = create_parsing_prompt(chunk, parse_description)
...
#                 print(f"✅ Chunk {i} completed in {processing_time:.2f} seconds")
                
#                 # Extract text from response
#                 if response.text and response.text.strip():
#                     result_text = response.text.strip()
                    
#                     # Skip if it's just saying no information found
#                     if "no relevant information found" not in result_text.lower():
#                         parsed_results.append(f"--- From Section {i} ---\n{result_text}")
...
#                 error_msg = f"Error processing section {i}: {str(e)}"
...
#         if parsed_results:
#             combined_result = "\n\n".join(parsed_results)
#             print(f"🎉 Parsing completed! Total results length: {len(combined_result)}")
...
#         return f"Error: {str(e)}. Please check your Google API key and internet connection."
...
class GeminiParser:
    def __init__(self, cache_dir="gemini_cache", cache_duration_hours=24, rate_limit_delay=4):
        ...
        print(f"🧠 Gemini Parser initialized with {rate_limit_delay}s rate limit")
    ...
    def _handle_quota_error(self, error_msg: str, attempt: int) -> int:
        ...
        if "429" in str(error_msg) or "quota" in str(error_msg).lower():
            ...
            print(f"🚨 Quota exceeded! Waiting {wait_time} seconds before retry (attempt {attempt + 1})")
...
6. **No Match Found**: If no matching information is found, return an empty string ('')"""
        ...
        while attempt <= max_retries:
            try:
                ...
                if "429" in error_msg or "quota" in error_msg.lower():
                    ...
                    # Non-quota error
                    if attempt < max_retries:
                        print(f"⚠️ Retrying in 5 seconds... (attempt {attempt + 2})")
                        time.sleep(5)
                        attempt += 1
                    else:
    ...
    def clear_old_cache(self, older_than_hours: int = 48):
        ...
        print(f"🧹 Cleared {cleared} old cache files (older than {older_than_hours}h)")
...
def parse_with_gemini(
    ...
    try:
        ...
        parser = GeminiParser(rate_limit_delay=5)  # 5 seconds = 12 calls/minute (safe buffer)
        
        # Show cache stats
        stats = parser.get_cache_stats()
        print(f"📊 Cache stats: {stats['cached_chunks']} cached chunks, {stats['cache_size_mb']} MB")
            ...
            try:
                ...
                if result and result.strip() and not result.startswith("❌"):
                    ...
                    if "Max retries reached" in result:
                        ...
                        parsed_results.append(f"--- Processing stopped at section {i} due to quota limits ---")
                        break
...
def test_gemini_connection():
    ...
    try:
        parser = GeminiParser()
        response = parser.model.generate_content("Say 'Hello from Gemini!' and nothing else.")
        return True, response.text.strip()
    except Exception as e:
        return False, str(e)
